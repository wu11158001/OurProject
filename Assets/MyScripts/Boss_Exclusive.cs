...
public class Boss_Exclusive : MonoBehaviourPunCallbacks
{
    ...
    void OnAttack3_Boss()
    {
        ...
        AttackMode attack = AttackMode.Instance;
        attack.performCharacters = gameObject;//執行攻擊腳色
        attack.performObject = GameSceneManagement.Instance.OnRequestOpenObject(GameSceneManagement.Instance.OnGetObjectNumber("bossAttack3"), GameSceneManagement.Instance.loadPath.bossAttack3);//執行攻擊的物件(自身/射出物件)
        attack.layer = "Enemy";//攻擊者layer
        attack.isCritical = isCritical;//是否爆擊

        attack.function = new Action(attack.OnSetShootFunction_Group);//設定執行函式       
        attack.damage = NumericalValue.bossAttack3_Damge * rate;//造成傷害 
        attack.direction = NumericalValue.bossAttack3_RepelDirection;//擊退方向(0:擊退, 1:擊飛)
        attack.repel = NumericalValue.bossAttack3_RepelDistance;//擊退/擊飛距離
        attack.animationName = NumericalValue.bossAttack3_Effect;//攻擊效果(播放動畫名稱)        

        attack.flightSpeed = NumericalValue.bossAttack3_FloatSpeed;//飛行速度
        attack.lifeTime = NumericalValue.bossAttack2_LifeTime;//生存時間
        attack.flightDiration = -transform.up;//飛行方向        
